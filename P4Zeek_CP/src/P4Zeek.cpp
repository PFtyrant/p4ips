...
struct timespec pa_start, pa_end, wait_start, wait_end, NN_start, NN_end,
    Mod_start, Mod_end, diff_tt, pin1_tt, pin2_tt, pin3_tt;
...
class malware_detection_Key {  // 用struct做key group
 public:
  ...
  bool operator<(const malware_detection_Key &k1) const {
    return (((k1.srcIP < this->srcIP) ||
             ((k1.srcIP == this->srcIP) && (k1.dstIP < this->dstIP)) ||
             ((k1.srcIP == this->srcIP) && (k1.dstIP == this->dstIP) &&
              (k1.srcport < this->srcport)) ||
             ((k1.srcIP == this->srcIP) && (k1.dstIP == this->dstIP) &&
              (k1.srcport == this->srcport) && (k1.dstport < this->dstport)) ||
             ((k1.srcIP == this->srcIP) && (k1.dstIP == this->dstIP) &&
              (k1.srcport == this->srcport) && (k1.dstport == this->dstport) &&
              (k1.protocol < this->protocol))));
  }
};

struct CmpFunc {
  bool operator()(const struct malware_detection_Key k1,
                  const struct malware_detection_Key k2) const {
    // return (((k1.srcIP!=k2.srcIP) || (k1.dstIP != k2.dstIP) || (k1.srcport !=
    // k2.srcport) || (k1.dstport != k2.dstport) || (k1.protocol !=
    // k2.protocol)));
    return (!((k1.srcIP == k2.srcIP) && (k1.dstIP == k2.dstIP) &&
              (k1.srcport == k2.srcport) && (k1.dstport == k2.dstport) &&
...
class sniff_fivetuple {  // total 16 bytes + 12 + 4
 ...
 public:
  ...
  bool operator<(const sniff_fivetuple &k1) const {
    return (
        ((k1.src_addr < this->src_addr) ||
         ((k1.src_addr == this->src_addr) && (k1.dst_addr < this->dst_addr)) ||
         ((k1.src_addr == this->src_addr) && (k1.dst_addr == this->dst_addr) &&
          (k1.src_port < this->src_port)) ||
         ((k1.src_addr == this->src_addr) && (k1.dst_addr == this->dst_addr) &&
          (k1.src_port == this->src_port) && (k1.dst_port < this->dst_port)) ||
         ((k1.src_addr == this->src_addr) && (k1.dst_addr == this->dst_addr) &&
          (k1.src_port == this->src_port) && (k1.dst_port == this->dst_port) &&
          (k1.protocol < this->protocol))));
  }
...
void childHandler(int signo) {
  ...
  while (waitpid(-1, &status, WNOHANG) >
         0) {  // 收屍目前掉剩下的小孩，如果fork失敗就會handler
    // do nothing
  }
}
...
void mongodb_initial() {
  ...
  if ((mongodb_pid = fork()) < 0) {
    ...
    close(mongodb_fd[0]);
    char *param[] = {(char *)"python3", (char *)"send_mongodb.py",
                     (char *)NULL};
...
void GetTS(uint64_t *global_timestamp) {  // just get timestamp but calculate
  auto bf_status = bf_ts_global_baresync_ts_get(
      dev_tgt.dev_id, global_timestamp,
      &baresync_ts_ns);  // it is real time to get??
  // auto bf_status =  bf_ts_global_baresync_ts_get(dev_tgt.dev_id,
  // &global_timestamp, &baresync_ts_ns);  // it is real time to get??
  // auto bf_status =  bf_ts_global_ts_value_get(dev_tgt.dev_id,
  // &global_timestamp);  // it is real time to get??
  assert(bf_status == BF_SUCCESS);
  // printf("time : %lu \n", global_timestamp);
}
...
int NN_Prediction(const uint8_t *payload_ptr) {
  ...
  // double time_used = (double) diff_tt.tv_sec*1000000000 + (double)
  // diff_tt.tv_nsec;
...
void tableSetUp() {
  ...
  auto bf_status = bfrtInfo->bfrtTableFromNameGet("Ingress.malware_detection",
                                                  &malware_detection);
  assert(bf_status == BF_SUCCESS);

  // Get action Ids for hit and miss actions
  //   bf_status =
  //       malware_detection->actionIdGet("Ingress.forward",
  //       &forward_action_id);
  //   assert(bf_status == BF_SUCCESS);
  ...
  bf_status =
      malware_detection->keyFieldIdGet("meta.src_port", &src_port_key_field_id);
  assert(bf_status == BF_SUCCESS);

  bf_status =
      malware_detection->keyFieldIdGet("meta.dst_port", &dst_port_key_field_id);
  assert(bf_status == BF_SUCCESS);
  ...
  bf_status = malware_detection->dataFieldIdGet(
      "yes",
      drop_action_id,  // dataField means parameter 跟上面的keyField不一樣
      &drop_field_id);
  assert(bf_status == BF_SUCCESS);
  ...
  **********************************************************************/
  ...
  // Allocate key and data once, and use reset across different uses // Allocate
  // should be installed
  bf_status = malware_detection->keyAllocate(
      &malware_bfrtTableKey);  // make key be added into table
  assert(bf_status == BF_SUCCESS);
  ...
  // bf_status =
  // malware_detection->dataAllocate(&malware_bfrtTableData_waiting);
  // assert(bf_status == BF_SUCCESS);
}
...
void malware_detection_key_setup(const malware_detection_Key &malware_key,
...
// void malware_detection_data_setup_for_drop(const malware_detectionData_drop
// &malware_data,
void malware_detection_data_setup_for_drop(
    const malware_detectionData &malware_data,
    bfrt::BfRtTableData *table_data) {
  auto bf_status = table_data->setValue(
      drop_field_id, static_cast<uint64_t>(malware_data.set_yes));
  assert(bf_status == BF_SUCCESS);
}

void malware_detection_drop_entry_add(
    const malware_detection_Key &malware_key,
    const malware_detectionData &malware_data) {
  malware_detection->keyReset(malware_bfrtTableKey.get());
  malware_detection->dataReset(drop_action_id,
                               malware_bfrtTableData_drop.get());
  malware_detection_key_setup(malware_key, malware_bfrtTableKey.get());
  malware_detection_data_setup_for_drop(malware_data,
                                        malware_bfrtTableData_drop.get());
...
void malware_detection_data_setup_for_NoAction(
    const malware_detectionData &malware_data,
    bfrt::BfRtTableData *table_data) {
  auto bf_status = table_data->setValue(
      NoAction_field_id, static_cast<uint64_t>(malware_data.set_yes));
  assert(bf_status == BF_SUCCESS);
}

void malware_detection_NoAction_entry_add(
    const malware_detection_Key &malware_key,
    const malware_detectionData &malware_data) {
  // std::shared_ptr<bfrt::BfRtTableKey>
  // malware_bfrtTableKey_temp(move(malware_bfrtTableKey));
  // std::shared_ptr<bfrt::BfRtTableData>
  // malware_bfrtTableData_NoAction_temp(move(malware_bfrtTableData_NoAction));
  // std::cout << "test 673\n";
  // malware_detection ->keyReset(malware_bfrtTableKey_temp.get());
  // std::cout << "test 675\n";
  // malware_detection ->dataReset(NoAction_action_id,
  // malware_bfrtTableData_NoAction_temp.get());
  malware_detection->keyReset(malware_bfrtTableKey.get());
  malware_detection->dataReset(NoAction_action_id,
                               malware_bfrtTableData_NoAction.get());
  // std::cout << "test 679\n";
  // malware_detection_key_setup(malware_key, malware_bfrtTableKey_temp.get());
  // std::cout << "test 681\n";
  // malware_detection_data_setup_for_NoAction(malware_data,
  // malware_bfrtTableData_NoAction_temp.get());
  // std::cout << "test 683\n";
  malware_detection_key_setup(malware_key, malware_bfrtTableKey.get());
  malware_detection_data_setup_for_NoAction(
      malware_data, malware_bfrtTableData_NoAction.get());

  auto bf_status =
      malware_detection
          ->tableEntryAdd(  // add entry here  here will add overlaped entry
              // auto bf_status = malware_detection->tableEntryMod(  // add
              // entry here  here will add overlaped entry
              // *session, dev_tgt, *malware_bfrtTableKey_temp,
              // *malware_bfrtTableData_NoAction_temp);
              *session, dev_tgt, *malware_bfrtTableKey,
              *malware_bfrtTableData_NoAction);
  // bf_status = malware_detection->tableEntryDel(*session, dev_tgt,
  // *malware_bfrtTableKey);
  // std::cout << "test 692\n";
  // assert(bf_status == BF_SUCCESS);
...
/*
void malware_detection_NoAction_entry_get(const malware_detection_Key
&malware_key, const BfRtTable::BfRtTableGetFlag &flag){
  malware_detection ->keyReset(malware_bfrtTableKey.get());
  // malware_detection ->dataReset(NoAction_action_id,
malware_bfrtTableData_NoAction.get());
  malware_detection_key_setup(malware_key, malware_bfrtTableKey.get());
...
malware_bfrtTableData_NoAction.get());
  ...
  auto bf_status = malware_detection->tableEntryGet(
    *session, dev_tgt, *malware_bfrtTableKey, flag,
malware_bfrtTableData_NoAction.get());
  bf_status = malware_bfrtTableData_NoAction->actionIdGet(&NoAction_action_id);

  malware_detectionData set_fwd_check;
  // bf_status = malware_bfrtTableData_NoAction->getValue(NoAction_action_id,
&set_fwd_check);
  // std::cout << "set_fwd_check.set_yes = " << set_fwd_check.set_yes <<
std::endl;

  // auto bf_status = malware_detection->tableEntryAdd(  // add entry here  here
will add overlaped entry
  //   *session, dev_tgt, *malware_bfrtTableKey,
*malware_bfrtTableData_NoAction);
}
*/

// void malware_detection_data_setup_for_waiting(const
// malware_detectionData_waiting &malware_data,
//                                       bfrt::BfRtTableData *table_data){
//   auto bf_status =
//       table_data->setValue(waiting_field_id,static_cast<uint64_t>(malware_data.set_yes));
//   assert(bf_status == BF_SUCCESS);
// }
// void malware_detection_waiting_entry_add(const malware_detection_Key
// &malware_key, const malware_detectionData_waiting &malware_data){
//   malware_detection ->keyReset(malware_bfrtTableKey.get());
//   malware_detection ->dataReset(waiting_action_id,
//   malware_bfrtTableData_waiting.get());
//   malware_detection_key_setup(malware_key, malware_bfrtTableKey.get());
//   malware_detection_data_setup_for_waiting(malware_data,
//   malware_bfrtTableData_waiting.get());

//   auto bf_status = malware_detection->tableEntryAdd(  // add entry here  here
//   will add overlaped entry
//     *session, dev_tgt, *malware_bfrtTableKey,
//     *malware_bfrtTableData_waiting);
...
void printfile() {
  ...
  // if(filevec.size() > 1){
  //   std::cout << "filevec[filevec.size()-1].ingress_ts = " <<
  //   filevec[filevec.size()-1].ingress_ts << std::endl;
  //   std::cout << "filevec[filevec.size()-2].ingress_ts = " <<
  //   filevec[filevec.size()-2].ingress_ts << std::endl;
  //   std::cout << "filevec.size() = " << filevec.size() << std::endl;
  //   if(filevec[filevec.size()-1].ingress_ts-filevec[filevec.size()-2].ingress_ts
  //   > 100000)  // delete useless information
  //     filevec.erase(filevec.end());
  // }
  int max_in2, min_in1;  // for ingress timestamp record
  outfile << "ingree_to_control NN_time control total_time ingress_ts "
             "control_start_ts control_end_ts ingress_ts2 label"
          << std::endl;
  for (int i = 0; i < filevec.size(); i++) {
    if (i < filevec.size()) {
      ...
      outfile << filevec[i].ingree_to_control << " " << filevec[i].NN_time
              << " " << filevec[i].control << " "
              << iter_f->second - filevec[i].ingress_ts << " "
              << filevec[i].ingress_ts << " " << filevec[i].control_start_ts
              << " " << filevec[i].control_end_ts << " " << iter_f->second
              << " " << filevec[i].label << std::endl;
      ...
      std::string str = std::string(inet_ntoa(srcIPStr)) + " " +
                        std::string(inet_ntoa(dstIPStr)) + " " +
                        std::to_string(filevec[i].f_tuple.srcport) + " " +
                        std::to_string(filevec[i].f_tuple.dstport) + " "
                        // + std::to_string(filevec[i].f_tuple.protocol) + " " +
                        // std::to_string(iter_f->second -
                        // filevec[i].ingress_ts) + " "
                        + std::to_string(filevec[i].f_tuple.protocol) + " " +
                        std::to_string(filevec[i].NN_time) + " " +
                        std::to_string(filevec[i].label) + " " +
                        std::to_string(filevec[i].ingress_ts) + " " +
                        std::to_string(iter_f->second);
  ...
  outfile << "\nave_ingree_to_control ave_NN_time ave_control_res "
             "ave_total_res correct_ave_NN_time"
          << std::endl;
  outfile << (uint64_t)(ave_ingree_to_control / valid_packetCount / 1000) << " "
          << (uint64_t)(ave_NN_time / valid_packetCount / 1000) << " "
          << (uint64_t)(ave_control_res / valid_packetCount / 1000) << " "
          << (uint64_t)(ave_total_res / valid_packetCount / 1000) << " "
          << (uint64_t)(total_NN_time / int(filevec.size())) << std::endl;
  outfile << "max_ingree_to_control max_NN_time max_control_res max_total_res"
          << std::endl;
  outfile << (uint64_t)(max_ingree_to_control / 1000) << " "
          << (uint64_t)(max_NN_time / 1000) << " "
          << (uint64_t)(max_control_res / 1000) << " "
          << (uint64_t)(max_total_res / 1000) << std::endl;
  iter_f = f_tuple_map.find(filevec[filevec.size() - 1].f_tuple);
  if (valid_packetCount != 0)
    outfile << "average total response time = "
            << (uint64_t)(max_in2 - min_in1) / valid_packetCount << std::endl;
  // outfile << "average total response time = " << (uint64_t)(iter_f->second -
  // filevec[0].ingress_ts)/valid_packetCount << std::endl;

  total_time_s +=
      start_get.utime + start_get.stime + start_get.cutime + start_get.cstime;
  total_time_e +=
      end_get.utime + end_get.stime + end_get.cutime + end_get.cstime;
  ...
  cpu_usage = 100 * ((double(total_time_e - total_time_s) / Hz) * 1000000000 /
                     total_time);
  double cpu_usage_sw = 100 * ((double(total_time_e - total_time_s) / Hz) *
                               1000000000 / total_time_sw);
  // outfile << "double((total_time_e-total_time_s) / Hz)*1000000000 = " <<
  // double((total_time_e-total_time_s) / Hz)*1000000000 << std::endl;

  outfile << std::endl;
  outfile << "total_time_s = " << total_time_s << std::endl;
  outfile << "total_time_e = " << total_time_e << std::endl;
  // std::cout << "diff_tt.tv_nsec = " << diff_tt.tv_nsec << "  |  " <<
  // "diff_tt.tv_sec = " << diff_tt.tv_sec << std::endl;
  ...
  outfile << "thread number : " << thread_number
          << "  |  cpu_usage : " << cpu_usage
          << "%  |  cpu_usage_sw : " << cpu_usage_sw << std::endl;
...
void calstat() {
  ...
  std::cout << "double((total_time_e-total_time_s) / Hz)*1000000000 = "
            << double((total_time_e - total_time_s) / Hz) * 1000000000
            << std::endl;
...
void packetHandler(const u_char *packet) {
  ...
  if (!(f_tuple->protocol == IPPROTO_UDP || f_tuple->protocol == IPPROTO_TCP)) {
    ...
    std::cout << "Packet in the record state\n";
    std::cout << "protocol number : " << f_tuple->protocol << "\n";
    printdata();
    printfile();
    // calstat();
  ...
  if (f_tuple->times == 2) {
    ...
    if (in_time == 0)
      ...
      //   f_tuple_map.insert(std::pair<malware_detection_Key,
      //   uint64_t>(five_tuple, in_time/1000));
      // }
    }
    mu3.unlock();
    // f_tuple_map[five_tuple] = in_time/1000;
    ...
    // if(iter_f == f_tuple_map.end()){
    //   f_tuple_map.insert(std::pair<malware_detection_Key,
    //   uint64_t>(five_tuple, in_time/1000)); // second in_time
    // }
    return;
  }

  // std::cout << "f_tuple->in_time1 = " << (int) f_tuple->in_time1 <<
  // std::endl;
  // std::cout << "f_tuple->in_time2 = " << (int) f_tuple->in_time2 <<
  // std::endl;
  // std::cout << "f_tuple->in_time3 = " << (int) f_tuple->in_time3 <<
  // std::endl;
  // std::cout << "f_tuple->in_time4 = " << (int) f_tuple->in_time4 <<
  // std::endl;
  // std::cout << "f_tuple->in_time5 = " << (int) f_tuple->in_time5 <<
  // std::endl;
  // std::cout << "f_tuple->in_time6 = " << (int) f_tuple->in_time6 <<
  // std::endl;
  // std::cout << "f_tuple->eg_time1 = " << (int) f_tuple->eg_time1 <<
  // std::endl;
  // std::cout << "f_tuple->eg_time2 = " << (int) f_tuple->eg_time2 <<
  // std::endl;
  // std::cout << "f_tuple->eg_time3 = " << (int) f_tuple->eg_time3 <<
  // std::endl;
  // std::cout << "f_tuple->eg_time4 = " << (int) f_tuple->eg_time4 <<
  // std::endl;
  // std::cout << "f_tuple->eg_time5 = " << (int) f_tuple->eg_time5 <<
  // std::endl;
  // std::cout << "f_tuple->eg_time6 = " << (int) f_tuple->eg_time6 <<
  // std::endl;
  ...
  // malware_detectionData set_fwd_check;
  // malware_detection_NoAction_entry_get(five_tuple,
  // BfRtTable::BfRtTableGetFlag::GET_FROM_SW);

  payload =
      (uint8_t *)(packet + SIZE_MIRROR + SIZE_ETHERNET + size_ip + size_tcp);
  // ++valid_packetCount;
  ...
  if (label == 0) {
    ...
    malware_detection_drop_entry_add(five_tuple,
                                     set_action);  // it is malware and drop
    malware_detection_drop_entry_add(five_tuple2,
                                     set_action);  // it is malware and drop
  }
  ...
  double time_used;
  if (five_tuple.srcport == 7 && five_tuple.dstport == 5001 &&
      five_tuple.protocol == 17)
    return;
  ...
  // in_time = ((uint64_t) f_tuple->in_time1)+((uint64_t)
  // f_tuple->in_time2)+((uint64_t) f_tuple->in_time3)+((uint64_t)
  // f_tuple->in_time4)+((uint64_t) (f_tuple->in_time5))+((uint64_t)
  // f_tuple->in_time6);
  ...
  // std::cout << "# " << seqhash << "   d_time = " << d_time << " ns" <<
  // std::endl;
}

// void threadhandler(const u_char* packet){
void threadhandler(u_char *userData, const struct pcap_pkthdr *pkthdr,
                   const u_char *packet) {
...
int parse2detect() {
  ...
  // descr = pcap_open_live(dev.c_str(), 65535, 1, -1, errbuf); //
  // 65535最大，表示不切割封包
  descr = pcap_open_live(dev.c_str(), 150, 1, -1,
                         errbuf);  // 65535最大，表示不切割封包
  if (descr == NULL) {
    std::cout << "pcap_open_live() failed: " << errbuf << std::endl;
    return 1;
  }

  // if (pcap_loop(descr, -1, packetHandler, NULL) < 0) { // so program stop
  // here with handler?
  if (pcap_loop(descr, -1, threadhandler, NULL) <
      0) {  // so program stop here with handler?
...
static void parse_options(bf_switchd_context_t *switchd_ctx,  // use for
  ...
  while (1) {
    int c = getopt_long(argc, argv, "h", options,
                        &option_index);  // &option_index means null???
...
//       datas temp;
//       temp.ingree_to_control = 1;
//       temp.control = 1;
//       temp.control_start_ts = 1;
//       temp.control_end_ts = 1;
//       temp.ingress_ts = 1;
//       temp.NN_time = 1;
//       temp.total_time = 1;

//       bfrt::examples::P4Zeek::GetTS();
//       temp.control_start_ts = bfrt::examples::P4Zeek::global_timestamp /
//       1000;
//       filevec.push_back(temp);
//       system("python /root/copy_SW18/01-simple_l3/pkt/send.py");
//       bfrt::examples::P4Zeek::GetTS();
//       temp.control_start_ts = bfrt::examples::P4Zeek::global_timestamp /
//       1000;
//       filevec.push_back(temp);
...
static void Init_bf_switchd(bf_switchd_context_t *switchd_ctx,
                            const char *progname) {
  ...
  // instead of dynamic configuration using static method.
  ...
  switchd_ctx->running_in_background = true;
  bf_status =
      bf_switchd_lib_init(switchd_ctx);  // to this line with some problems
}

int main(int argc, char **argv) {
  // pthread_t t1;

  const char *progname = "P4Zeek";
  bf_switchd_context_t *switchd_ctx;  // switch context
  Init_bf_switchd(switchd_ctx, progname);

  // parse_options(switchd_ctx, argc, argv);
  // bfrt::examples::P4Zeek::TS_STATE_CHECK();  // 他前面只不過加上namespace而已
  // my commit
  ...
  system("bfshell -f /root/ips_test/P4Zeek_CP/cmd/bfshell_commands.txt");
  system("bfshell -b /root/ips_test/P4Zeek_CP/cmd/bfrt_python_script.py");
